...
#include <algorithm>
using namespace std;
void dfs(int person, int n, int m, vector<int>& assignment, vector<bool>& bookUsed, 
         const vector<vector<int>>& like, vector<vector<int>>& solutions){
    if(person == n){
        solutions.push_back(assignment);
        return;
    }
    for(int book = 0; book < m; book++){
        if (!like[person][book] || bookUsed[book]) continue;
        assignment[person] = book;
        bookUsed[book] = true;
        dfs(person + 1, n, m, assignment, bookUsed, like, solutions);
        bookUsed[book] = false;
    }
}
int main(){
    ...
    vector<bool> bookUsed(m, false);
    vector<vector<int>> solutions;
    dfs(0, n, m, assignment, bookUsed, like, solutions);
